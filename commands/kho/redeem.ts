...
export async function modalRun(interaction: ModalSubmitInteraction) {
	...
	interaction.followUp("Your feedback was sent!");
}
...
export async function buttonRun(interaction: ButtonInteraction) {
	...
	case "star":
		...
		interaction.reply({
			content: `You choosed **${star}** ${client.emotes.star}! Click **Feedback** button to send your feedback.`,
...
export async function execute(ctx: Context) {
	...
	if (!code) return ctx.sendMessage("Please provide the code to redeem.");
	...
	if (!validCode)
		return ctx.sendFollowUp("Invalid code.");

	// Check stock
	const stock = db.stock.find(data => data.name === validCode.name);
	if (!stock)
		return ctx.sendFollowUp("Product not found.");

	if (stock.content.length < validCode.amount)
		return ctx.sendFollowUp("We don't have enough stock to send! Try again later.");
	...
	ctx.author.send({
		embeds: [{
			description: "- **Product** \n [Information and usage](http://home.monsterservice.xyz/) "
				+ "\n```"
				+ send.join("\n")
				+ "```",
			color: Colors.Blue,
		...
		ctx.sendFollowUp("You have activated the code and received the product successfully!");
		...
		ctx.sendFollowUp("I can't send the product to you.");
	});
}
